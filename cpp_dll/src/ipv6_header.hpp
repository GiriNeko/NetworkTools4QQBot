...
public:
    ...
    unsigned char traffic_class()  const noexcept { return ((rep_[0] & 0xF) << 4) | (rep_[1] >> 4) & 0xF; }
    unsigned int flow_label() const noexcept { return (rep_[1] & 0xF) << 16 | (rep_[2] << 8) | rep_[3]; }
    unsigned short payload_length() const noexcept { return (rep_[4] << 8) + rep_[5]; }
