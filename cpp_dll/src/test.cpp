...
std::vector<tracert_compose> tracert(const std::string& dest)
{
    ...
    for (auto ttl: std::views::iota(1) | std::views::take(30))
    {
        ...
        for (const auto& [ipv4_hdr, _1, length, elapsed]: future.get())
        {
            ...
            local_list[idx++] = length ? std::chrono::duration_cast<std::chrono::milliseconds>(elapsed).count() : -1;
        }
        list.emplace_back(ttl, local_list[0], local_list[1], local_list[2], address.to_string());
